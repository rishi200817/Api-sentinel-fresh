...
import { buildContext, migrationFor } from "@/lib/sentinel/agent/orchestrator";
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const result = await migrationFor(change, ctx);
    return ok(result);
  } catch (err) {
    return fail(err instanceof Error ? err.message : "Migration generation failed.", 500);
  }
}
